...
const Card = ({ item, isAnime = false }) => {
	const navigate = useNavigate();
	return (
		<div
			className='h-72 w-44 hover:scale-95 pointer-events-auto'
			key={item.id}
			onClick={() => navigate(isAnime ? `/anime/${item.id}` : `/stream/${item.episodeId}`)}
		>
			...
			<div className='p-2 text-center bg-zinc-800 h-20'>
				...
				{item?.totalEpisodes && (
					<div className='text-zinc-500 text-sm gap-1 '>Total:{item?.totalEpisodes}</div>
				)}
				{item?.episodeNumber && (
